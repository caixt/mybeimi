 ...
 *
 */
public class DuZhuBoard extends Board {
	...
	public TakeCards takeCardsRequest(GameRoom gameRoom , Board board, Player player,
		...
		if(takeCards!=null){		//通知出牌
			 ...
			 */
			if(board.isWin()){//出完了
				GameUtils.getGame(gameRoom.getPlayway()).change(gameRoom , BeiMiGameEvent.ALLCARDS.toString() , 0);	//赢了，通知结算
				takeCards.setNextplayer(null);
			}else{
				PlayUserClient nextPlayUserClient = ActionTaskUtils.getPlayUserClient(gameRoom.getId(), takeCards.getNextplayer()) ;
				if(BMDataContext.PlayerTypeEnum.NORMAL.toString().equals(nextPlayUserClient.getPlayertype())){
					GameUtils.getGame(gameRoom.getPlayway()).change(gameRoom , BeiMiGameEvent.PLAYCARDS.toString() , 25);	//应该从 游戏后台配置参数中获取
				}else{
					GameUtils.getGame(gameRoom.getPlayway()).change(gameRoom , BeiMiGameEvent.PLAYCARDS.toString() , 3);	//应该从游戏后台配置参数中获取
				}
			}
		}else{
			takeCards = new TakeDiZhuCards();
